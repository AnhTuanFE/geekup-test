...
import { CheckCircleTwoTone, MinusSquareTwoTone, MenuOutlined } from '@ant-design/icons';
import styles from './Todo.module.css';
import useFetchUsers from '../../hooks/dataUsers/useFetchUsers';
import { Divider, Select, Button, Spin } from 'antd';
...
function Todo() {
        ...
        fetch(`https://jsonplaceholder.typicode.com/users/${idUser}/todos`)
            ...
            .then((data) => {
                ...
                if (dataSort.length > 0) {
                    setStoreData((prevData) => [...prevData, { id: dataSort[0].userId | null, listTask: dataSort }]);
                }
            })
            .finally(() => {
    ...
    return (
        <div className="px-10 pb-16 pt-[90px]">
            <div className="">
            ...
            <div className={clsx(styles.title)}>
                <Divider className={clsx(styles.task_title)} orientation="left" orientationMargin="0">
                    Task
                </Divider>
            </div>
                                ...
                                return (
                                    <li className={clsx(styles.wrap_data_item)} key={item.id}>
                                        <span className={clsx(styles.data_item)}>
                                            ...
                                            <p className={clsx(styles.data_item_child)}>{item.title}</p>
                                        </span>
                                        {!item.completed && (
            ...
            <div className={clsx(styles.data_task)}>
                Done {task?.filter((item) => item.completed === true).length}/{task?.length} tasks
            </div>
